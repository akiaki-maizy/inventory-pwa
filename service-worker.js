const CACHE_NAME = 'inventory-pwa-v1-7-case-stock';
...
async function fixedIndexResponse(request){
 try{
  ...
  html=html.replace('MVP Ver.1.4 / 端末内保存','MVP Ver.1.7 / ケース・バラ在庫 / 先期限先出し使用 / 端末内保存');
  html=html.replace('MVP Ver.1.5 / JAN読取対応 / 端末内保存','MVP Ver.1.7 / ケース・バラ在庫 / 先期限先出し使用 / 端末内保存');
...
self.addEventListener('fetch',event=>{
 ...
 if(url.pathname.endsWith('/jan-scanner.js')||url.pathname.endsWith('/case-stock.js')){
